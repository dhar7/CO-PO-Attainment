...
//$filename = "Assignment_"."2K".$batch."_".$code.".csv";;  // Replace with the path to your CSV file
    ...
    for( $i = 0; $i<16; $i++)
    {
        if( ($data[0] != '') && ($data[0] != 'Roll') )
        {
            ...
            else
            {
                if ($matrix[2][$i-1] == 0)
                {
                    if ( (int)$data[$i] <= (int)$matrix[3][$i-1])
                    {
                        $obatained[$r][1] += (int)$data[$i];
                        $total[$r][1] += (int)$matrix[3][$i-1];
                    }
                    else
                    {
                ...
                elseif ($matrix[2][$i-1] == 1)
                {
                    if ( (int)$data[$i] <= (int)$matrix[3][$i-1])
                    {
                        $obatained[$r][2] += (int)$data[$i];
                        $total[$r][2] += (int)$matrix[3][$i-1];
                    }  
                    else
                    {
                        //header("Location: index.html?error=exceeded");
                ...
                elseif ($matrix[2][$i-1] == 2)
                {
                    if ( (int)$data[$i] <= (int)$matrix[3][$i-1])
                    {
                        $obatained[$r][3] += (int)$data[$i];
                        $total[$r][3] += (int)$matrix[3][$i-1];
                    }  
                    else
                    {
                        //header("Location: index.html?error=exceeded");
                ...
                elseif ($matrix[2][$i-1] == 3)
                {
                    if ( (int)$data[$i] <= (int)$matrix[3][$i-1])
                    {
                        $obatained[$r][4] += (int)$data[$i];
                        $total[$r][4] += (int)$matrix[3][$i-1];
                    }    
                    else
                    {
                        //header("Location: index.html?error=exceeded");
                ...
                elseif ($matrix[2][$i-1] == 4)
                {
                    if ( (int)$data[$i] <= (int)$matrix[3][$i-1])
                    {
                        $obatained[$r][5] += (int)$data[$i];
                        $total[$r][5] += (int)$matrix[3][$i-1];
                    }  
                    else
                    {
                        //header("Location: index.html?error=exceeded");
                ...
                elseif ($matrix[2][$i-1] == 5)
                {
                    if ( (int)$data[$i] <= (int)$matrix[3][$i-1])
                    {
                        $obatained[$r][6] += (int)$data[$i];
                        $total[$r][6] += (int)$matrix[3][$i-1];
                    } 
                    else
                    {
                        //header("Location: index.html?error=exceeded");
...
$_SESSION['assignment'] = $obatained;
$_SESSION['total_assignment'] = $total;

header("Location: calculate_term_final.php");
exit;
//exit;
